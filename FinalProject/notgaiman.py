...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "Я Нил Гейман. А может, нет. Проверим?\nЧтобы начать игру заново, отправь /start.\nМои сообщения не должны, но могут грузиться долго, потому что я обрабатываю четыре романа. Прости меня за это, все-таки я живу далеко, письма из Британии в Россию идут небыстро!")
    handle_text(message)
m = Mystem()
...
def go_out(message):
    bot.send_message(message.from_user.id, "Ну и ладно, до встречи когда-нибудь в следующий раз. Твой результат - " + str(sb[message.chat.id]) + '. Чтобы начать игру снова, отправь /start. Чтобы продолжить эту, нажми /да.')
